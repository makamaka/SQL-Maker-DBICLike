...
$cond = struct_to_cond( { foo => 'bar' }, { reduce_parentheses => 1 } );
...
$cond = struct_to_cond( { status => ['foo','bar'] }, { reduce_parentheses => 1 } );
is( $cond->as_sql, q{status IN (?, ?)} );
is_deeply( [ $cond->bind ], [qw/foo bar/] );

$cond = struct_to_cond( { status => [-or => 'foo','bar'] }, { reduce_parentheses => 1 } );
is( $cond->as_sql, q{(status = ?) OR (status = ?)} );
is_deeply( [ $cond->bind ], [qw/foo bar/] );

$cond = struct_to_cond( { status => [-and => 'foo','bar'] }, { reduce_parentheses => 1 } );
